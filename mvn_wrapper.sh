#!/usr/bin/bash
# mvn_wrapper
# Tested on GNU bash, version 4.2.24(1)-release
# bash 4.0 and higher required for custom aliases!

# where is mvn?
MW_MVN_BIN=`which mvn --skip-alias 2>/dev/null`

############################### Your Custom Aliases ###############################
declare -A custom_aliases
custom_aliases["--Server"]="clean install -U -Dmaven.test.sktip=true"

############################### Your Custom Options ###############################
custom_options="-Dcompress.js=false"
...
_mvn() {
	...
	if [[ ${cur} == -D* ]] ; then
	  COMPREPLY=( $(compgen -S ' ' -W "${options}|${custom_options}" -- ${cur}) )

	elif [[ ${cur} == -P* ]] ; then
	  COMPREPLY=( $(compgen -S ' ' -W "${profile_settings}|${profile_pom}" -- ${cur}) )

	elif [[ ${cur} == --* ]] ; then
	  COMPREPLY=( $(compgen -W "${long_opts}|${!custom_aliases[@]}" -S ' ' -- ${cur}) )
...
BOLD_TEXT=$(tput bold)
BLACK_TEXT=`echo -e "\E[30m"`
RED_TEXT=`echo -e "\E[31m"`
GREEN_TEXT=`echo -e "\E[32m"`
YELLOW_TEXT=`echo -e "\E[33m"`
BLUE_TEXT=`echo -e "\E[34m"`
MAGENTA_TEXT=`echo -e "\E[35m"`
CYAN_TEXT=`echo -e "\E[36m"`

RESET_TEXT="${BLACK_TEXT}`(tput sgr0)`"

MVN_INFO=$BLUE_TEXT
MVN_WARNING=$YELLOW_TEXT
MVN_ERROR=$RED_TEXT
MVN_SUCCESS=$GREEN_TEXT

##### Alias 'mvn' with this function that provides colors and custom aliases ######
function mvn-color-alias() {
	options=()
	while [[ $1 ]] ; do
		for i in "${custom_aliases[@]}" ; do
			if [ "$i" == "$1" ]; then
				options+=("{$custom_aliases[$i]}")
			else
				options+=("$1")
			fi
			shift
	...
	trap "echo" INT TERM QUIT
	"$MW_MVN_BIN" ${options[@]} | sed \
			-e "s/\(\[INFO\]\ .*\)/${MVN_INFO}\1${RESET_TEXT}/g"\
			-e "s/\(\[WARNING\]\ .*\)/${MVN_WARNING}\1${RESET_TEXT}/g"\
			-e "s/\(\[ERROR\]\ .*\)/${MVN_ERROR}\1${RESET_TEXT}/g"\
			-e "s/\(\[INFO\]\ .*SUCCESS.*\)/${MVN_SUCCESS}\1${RESET_TEXT}/g" 
	echo -ne ${RESET_TEXT}
}

alias mvn="mvn-color-alias"
